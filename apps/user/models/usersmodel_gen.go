...
func (m *defaultUsersModel) FindByPhone(ctx context.Context, phne string) (*Users, error) {
	usersIdKey := fmt.Sprintf("%s%v", cacheUsersIdPrefix, phne)
	var resp Users
	err := m.QueryRowCtx(ctx, &resp, usersIdKey, func(ctx context.Context, conn sqlx.SqlConn, v any) error {
		query := fmt.Sprintf("select %s from %s where `phne` = ? limit 1", usersRows, m.table)
		return conn.QueryRowCtx(ctx, v, query, phne)
	})
	switch err {
	case nil:
		return &resp, nil
